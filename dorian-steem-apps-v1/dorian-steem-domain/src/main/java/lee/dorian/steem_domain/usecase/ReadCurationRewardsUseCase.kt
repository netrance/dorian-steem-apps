...
class ReadCurationRewardsUseCase @Inject constructor(
    ...
    // Reads every curation reward paid to the account in the given time range, latest first.
    // rewards_api can only be read oldest first, so the whole range is read at once instead
    // of page by page, and every reward it holds is returned. The range is the only thing
    // that bounds the amount of work and the size of the result, which makes it the caller's
    // job to keep it small enough to be worth reading.
    suspend operator fun invoke(
        ...
        try {
            steemWorldRepository.readCurationRewards(account, fromTime, toTime)
        } catch (e: Exception) {
            e.printStackTrace()
            ApiResult.Error(e)
        }
